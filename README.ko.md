...
- **자막이 없는 영상에는 선택적 whisper 폴백이 필요합니다.** 기본적으로 tubeless는
  자막만 읽으므로, 자막이 아예 없는 영상은 건너뜁니다(다이제스트에서는 "Videos
  without a transcript"로 표시) — 다른 언어 자막만 있는 영상은 그대로 요약합니다.
  자막 언어가 요약 언어와 일치할 필요는 없기 때문입니다. 자막 없는 영상까지
  받아쓰려면 `tubeless[whisper]`를 설치하고 `TUBELESS_WHISPER_MODEL`(예: `small`)을
  설정하세요. 그러면 오디오를 내려받아 faster-whisper를 로컬에서 실행합니다. 이
  단계는 자막 경로가 피하는 무거운 작업이라 기본은 꺼져 있습니다.
